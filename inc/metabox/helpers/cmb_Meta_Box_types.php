...
class cmb_Meta_Box_types {
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. __( 'No terms', 'wp_pet' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function taxonomy_multicheck() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. __( 'No terms', 'wp_pet' ) .'</label> <a class="button button-primary button-small" href="edit-tags.php?taxonomy='.$this->field->args( 'taxonomy' ).'&post_type=pet/">'.__( 'Add', 'wp_pet' ).'</a></li>';
		} else {
